		...
		with open(fname, 'w',newline='\n') as gps_out:
			...
			while t.time() <= t_end or ipos < gps_samples or ivel < gps_samples:
				...
				if "GPGGA" in newline:
					...
					u[ivel] = 0.2778 * gpvtg.spd_over_grnd_kmph*np.sin(np.deg2rad(gpvtg.true_track)) #get u component of SOG and convert to m/s
					v[ivel] = 0.2778 * gpvtg.spd_over_grnd_kmph*np.cos(np.deg2rad(gpvtg.true_track)) #get v component of SOG and convert to m/s
					ivel+=1
